...
for mode in modes
    @show mode
    for low_dimension in 20:35
        for high_dimension in 500:20:600
            for seed in 1:10
                    ...
                    for alternative in alternatives
                        for decision_function in decision_functions
                            ...
                            for weight in weights
                                @show seed, dimension
                                run(`sbatch batch_low_dim_high_dim.sh $mode $low_dimension $seed $alternative $decision_function $depth $weight $high_dimension`)
                            end
                        end
                    end
                end
